...
import { isIsoDay } from "@/lib/access";
...
export async function POST(request: NextRequest) {
  ...
  if (!title || !isIsoDay(day)) {
    return NextResponse.json({ error: "تاریخ را به شکل 2026-09-11 بنویسید." }, { status: 400 });
  }
...
export async function DELETE(request: NextRequest) {
  ...
  const removed = await db.delete(calendarEvents).where(eq(calendarEvents.id, id)).returning();
